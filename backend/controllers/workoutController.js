...
const getWorkouts = async (req, res) => {
    ...
    const workouts = await Workout.find({ user_id }).sort({createdAt: -1}) // descending order of all workouts
...
const createWorkout = async (req, res) => {
    ...
    try {
        const user_id = req.user._id
        const workout = await Workout.create({title, load, reps, user_id}) // creates the workout document
        res.status(200).json(workout)
    } catch (error){
